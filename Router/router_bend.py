# Router with bend penalty consideration
import numpy as np
import heapq

g = open("bench5.grid","r")
g = g.read().split("\n")
                ...
                if(x < X-1):
                    if(grid[x+1,y,l]!=-1):
                        if((pred[x,y,l]==1)|(pred[x,y,l]==2)):
                            heapq.heappush(wavefront,(curr[0]+grid[x+1,y,l]+b_penalty,l+1,x+1,y))
                        else:
                ...
                if(y > 0):
                    if(grid[x,y-1,l]!=-1):
                        if((pred[x,y,l]==3)|(pred[x,y,l]==4)):
                            heapq.heappush(wavefront,(curr[0]+grid[x,y-1,l]+b_penalty,l+1,x,y-1))
                        else:
